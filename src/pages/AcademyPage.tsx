...
const AcademyPage = () => {
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Hero */}
      <section className="pt-28 md:pt-36 pb-20 border-b border-border">
        <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20">
          <div className="flex items-center gap-2 mb-5">
            <Link to="/" className="text-muted-foreground hover:text-foreground text-xs tracking-widest uppercase font-body transition-colors">Home</Link>
            <span className="text-muted-foreground/40 text-xs">›</span>
            <span className="label-text text-xs">Academy</span>
          </div>

          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-xs tracking-widest text-muted-foreground uppercase font-body mb-4">
            INCISE ACADEMY — EAST AFRICA'S SURFACE CRAFT SCHOOL
          </motion.p>

          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="font-display text-5xl md:text-7xl lg:text-8xl text-foreground leading-[0.95] mb-6">
            Certify. Apply.
            <br />
            <span className="italic text-gradient-gold">Build.</span>
          </motion.h1>

          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-muted-foreground font-body max-w-2xl leading-relaxed mb-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="flex flex-wrap gap-4 mb-14">
            <a href="#intakes" className="bg-primary text-primary-foreground px-8 py-3.5 text-xs tracking-wider font-body hover:bg-gold-light transition-colors">APPLY FOR CERTIFICATION →</a>
            <a href="#programme" className="border border-border text-foreground px-8 py-3.5 text-xs tracking-wider font-body hover:border-primary hover:text-primary transition-colors">EXPLORE THE PROGRAMME</a>
          </motion.div>

          {/* Quote */}
          <motion.blockquote initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }} className="border-l-2 border-primary pl-6 max-w-2xl">
            <p className="font-display italic text-lg text-muted-foreground leading-relaxed">
            ...
            <cite className="text-xs text-muted-foreground/50 font-body tracking-wider uppercase mt-3 block not-italic">— INCISE OPERATIONS BIBLE</cite>
          </motion.blockquote>
        </div>
      </section>
      ...
      <section className="section-padding">
        <div className="max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-secondary-foreground font-body leading-relaxed mb-6 max-w-3xl">
            ...
            <p className="text-secondary-foreground font-body leading-relaxed mb-8 max-w-3xl">
            ...
            <ul className="space-y-3 max-w-3xl">
              {["Formal 12-week certification structured across three phases", "Modules covering brand philosophy, technical application, QA systems, and business development", "Supervised live project deployment — real clients, real stakes", "Five progression tiers from Certified Applicator to Certified Trainer", "Annual recertification to maintain standards", "Graduates supported by INCISE's ongoing technical team"].map((item, i) => (
                <li key={i} className="flex gap-3 text-sm text-secondary-foreground font-body">
      ...
      <section className="section-padding bg-secondary">
        <div className="max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            ...
            <p className="text-muted-foreground font-body mt-4 max-w-lg mx-auto text-sm">
          ...
          <div className="space-y-0">
            {tiers.map((tier, i) => (
              <motion.div key={tier.num} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="flex gap-6 md:gap-10 py-8 border-t border-border group">
                <span className="text-4xl md:text-5xl font-display text-primary/20 group-hover:text-primary transition-colors flex-shrink-0">{tier.num}</span>
                <div>
                  <p className="label-text text-[10px] mb-1">{tier.tier}</p>
                  <h3 className="font-display text-xl md:text-2xl text-foreground mb-2">{tier.title}</h3>
                  <p className="text-muted-foreground font-body text-sm leading-relaxed">{tier.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section id="programme" className="section-padding">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="space-y-10">
            {phases.map((phase, i) => (
              <motion.div key={phase.num} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-secondary border border-border p-8 md:p-10">
                <div className="flex items-start gap-6 mb-6">
                  <span className="text-4xl font-display text-primary/30">{phase.num}</span>
                  <div>
                    <p className="label-text text-[10px] mb-1">{phase.weeks}</p>
                    <h3 className="font-display text-2xl md:text-3xl text-foreground">{phase.title}</h3>
                  </div>
                </div>
                <p className="text-secondary-foreground font-body leading-relaxed mb-6 text-sm">{phase.desc}</p>
      ...
      <section className="section-padding bg-secondary">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {curriculum.map((c, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="bg-muted border border-border p-6 hover:border-primary/30 transition-colors group">
                <span className="text-3xl mb-3 block">{c.icon}</span>
                <h3 className="text-foreground font-body font-semibold text-sm mb-2 group-hover:text-primary transition-colors">{c.title}</h3>
      ...
      <section id="intakes" className="section-padding">
        <div className="max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-4">
            ...
            <a href="#" className="text-xs text-muted-foreground hover:text-primary font-body tracking-wider transition-colors">CAN'T FIND A DATE? CONTACT US →</a>
          </motion.div>

          <div className="space-y-4 mt-12">
            {intakes.map((intake, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-secondary border border-border p-6 md:p-8 flex flex-col md:flex-row gap-6 items-start">
                <div className="flex-shrink-0 text-center md:text-left">
                  <p className="font-display text-5xl text-primary leading-none">{intake.day}</p>
                  <p className="text-xs text-muted-foreground tracking-widest uppercase font-body mt-1">{intake.month}</p>
                </div>
                <div className="flex-1">
                  <h3 className="font-display text-xl text-foreground mb-2">{intake.title}</h3>
                  <p className="text-sm text-muted-foreground font-body mb-3">{intake.details}</p>
                  <p className="label-text text-[10px] mb-4">{intake.status}</p>
                  <a href="#" className="inline-flex bg-primary text-primary-foreground px-6 py-2.5 text-xs tracking-wider font-body hover:bg-gold-light transition-colors">
                    {intake.cta}
                  </a>
                </div>
              </motion.div>
      ...
      <section className="section-padding bg-secondary">
        <div className="max-w-4xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            ...
            <h2 className="font-display text-3xl md:text-4xl text-foreground">
              What our graduates <span className="italic text-gradient-gold">say.</span>
            </h2>
          </motion.div>

          <div className="space-y-8">
            {testimonials.map((t, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-muted border border-border p-8">
                <p className="font-display text-4xl text-primary/20 leading-none mb-4">"</p>
      ...
      <section className="section-padding border-t border-border">
        <div className="max-w-3xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <p className="text-muted-foreground font-body text-sm mb-8 max-w-lg mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <a href="#intakes" className="bg-primary text-primary-foreground px-8 py-3.5 text-xs tracking-wider font-body hover:bg-gold-light transition-colors">APPLY FOR CERTIFICATION →</a>
              <a href="/technical" className="border border-border text-foreground px-8 py-3.5 text-xs tracking-wider font-body hover:border-primary hover:text-primary transition-colors">VIEW TECHNICAL LIBRARY</a>
            </div>
          </motion.div>
        </div>
      </section>
